...
from .src.coinbase_agentkit import (
    AgentKit, 
    AgentKitOptions, 
...
def main():
    ...
    options = AgentKitOptions(
        ...
        action_providers=[pyth_action_provider()]
    )
    agent_kit = AgentKit(options)

    try:
        # Get BTC price feed ID
        feed_id_result = agent_kit.get_actions()[0].invoke({
            "token_symbol": "BTC"
        })
        print(f"BTC Price Feed ID: {feed_id_result}")

        # Get BTC price using the feed ID
        price_result = agent_kit.get_actions()[1].invoke({
            "price_feed_id": feed_id_result
        })
